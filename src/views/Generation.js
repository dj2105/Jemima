...
// Production: Gemini + Firestore seeding.
// - Creates/updates rooms/{code} (status: "generating").
// - Generates quiz questions, interludes, and maths via Netlify function.
...
export default function Generation(ctx = {}) {
  ...
  const keyRoom = safeJSON(lsGet('keyRoom', '{}'));
  ...
  sub.className = 'note';
  sub.textContent = 'Seeding questions and Jemima passages.';
  if (roomCode) sub.textContent += ` Room ${roomCode}.`;
